...
impl<
        TCommand: Clone + 'static + std::marker::Send + std::fmt::Debug,
        TLog: Clone + 'static,
        EReceiver: 'static,
    > GUI<TCommand, TLog, EReceiver>
{
    pub fn new(
        components: Vec<GUIComponent<TCommand, TLog>>,
        send_command: Box<dyn FnMut(TCommand)>,
                ...
                GUIComponent::CommandSendButton {
                    ...
                    column = column.push(
                        button(Text::new(label))
                            .on_press(Message::SendCommand(command_to_send.clone())),
                    );
                }
    ...
    pub fn view_test() {
        let components: Vec<GUIComponent<TestData, TestData>> = vec![
            GUIComponent::TextView {
                ...
                new_text: Box::new(|log, _| log.data.clone()),
                text: String::from("test1"),
            },
            GUIComponent::CommandSendButton {
                label: String::from("test button"),
                command_to_send: TestData {
...
pub mod test {
    ...
    fn test_send() {
        ...
        let mut gui: super::GUI<TestData, TestData, ()> = super::GUI::new(
            components,
            Box::new(move |c| *send_data_clone.borrow_mut() = Some(c)),
            Box::new(|| {
                Ok(TestData {
                    data: String::new(),
                })
        ...
        gui.update(super::Message::SendCommand(TestData {
    ...
    #[test]
    fn test_receive() {
        let components = vec![GUIComponent::TextView {
            label: String::from("test"),
            new_text: Box::new(|log: TestData, _| log.data),
            text: String::from("ng"),
        }];
        ...
        let mut gui: super::GUI<TestData, TestData, ()> = super::GUI::new(
            ...
            Box::new(|_| {}),
            Box::new(move || {
                *receive_count_clone.borrow_mut() += 1;
                Ok(TestData {
                    data: String::from("ok"),
                })
            }),
